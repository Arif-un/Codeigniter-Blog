<?php /* check_Auth */ !isset($this->session->userdata['logged_in']) ? redirect(base_url()) : ""; ?>
    <body>
        <header>
            <a href="<?=base_url()?>" class="title logo">Codeigniter Blog</h1>
            </a>

            <div class="is-pulled-right">


                <button class="profile-nav button is-white is-rounded">
                    <span class="ti-search is-size-5 has-text-primary"></span>
                </button>
                <button class="profile-nav button is-white is-rounded">
                    <span class="ti-bookmark-alt is-size-5 has-text-primary"></span>
                </button>

                <div class="dropdown is-right">
                    <div class="dropdown-trigger">
                        <button id="notify" class="profile-nav button is-white is-rounded dropdown-btn" aria-haspopup="true" aria-controls="dropdown-menu">
                            <span class="ti-bell is-size-5 has-text-primary"></span>
                        </button>
                    </div>
                    <div class="dropdown-menu" id="dropdown-menu" role="menu">
                        <div class="dropdown-content">
                            <a href="#" class="dropdown-item">
                                No Notification
                            </a>

                        </div>
                    </div>
                </div>

                <div class="dropdown is-right">
                    <div class="dropdown-trigger">
                        <button id="profile" class="profile-nav button is-white is-rounded dropdown-btn" aria-haspopup="true" aria-controls="dropdown-menu">
                            <img class="profile-avater" src="data:image/png;base64,"
                                alt="">&nbsp
                            <span class="ti-angle-down is-size-7 has-text-primary"></span>
                        </button>
                    </div>
                    <div class="dropdown-menu" id="dropdown-menu" role="menu">
                        <div class="dropdown-content">
                            <a href="<?=base_url()?>index.php/Posts/newPost" class="dropdown-item">
                                New Post
                            </a>
                            <a class="dropdown-item">
                                Other Posts
                            </a>
                            <hr class="dropdown-divider">
                            <a href="<?=base_url()?>index.php/profile" class="dropdown-item">
                                Profile
                            </a>
                            <a href="#" class="dropdown-item">
                                Settings
                            </a>
                            <a href="#" class="dropdown-item">
                                Help
                            </a>
                            <a href="<?= base_url()?>index.php/signout" class="dropdown-item">
                                Sign Out
                            </a>
                        </div>
                    </div>
                </div>

            </div>
        </header>

        <section class="wrapper">

            <div class="profile-head circle-effect">
                <div class="">
                    <h3 class="is-size-3 has-text-weight-bold"><?=$_SESSION['logged_in']['name']?></h3>
                    <h6 class="is-size-5">Bio goes here</h6>

                    <P style="margin:10px 0 auto 0" class="is-size-7 has-text-grey-light">47 Following &nbsp; 2k Follower</P>
                </div>
                <button class="button is-outlined  has-text-grey is-rounded is-small">Edit Profile</button>
                <img src="data:image/png;base64,"
                    alt="">
            </div>

            <div style="padding:30px 30px 30px 30px;">
                <a class="has-text-grey">
                    <span class="ti-user"></span> Profile</a> &nbsp;
                <a class="has-text-grey">
                    <span class="ti-thumb-up"></span> Thumb Ups</a>
                <hr class="is-marginless">
            </div>

            <div style="padding:0 30px 0 30px">
                <h5 class="is-size-5 has-text-weight-bold">Posts</h5>

                <?php foreach($posts as $post) : ?>

                <div class="profile-posts">

                    <div class="is-flex">
                        <img src="data:image/png;base64,"
                            alt="">
                        <div>
                            <p><?=$_SESSION['logged_in']['name']?></p>
                            <p class="is-size-7 has-text-grey-light"><?=nice_date($post->created_at, 'M d')?> · <?=$post->length_in_minute?> min</p>
                        </div>
                    </div>

                    <h2 class="is-size-3 has-text-weight-semibold"><a class="has-text-grey-darker" href="<?=base_url().'index.php/posts/'.$post->id?>"><?= $post->title?></a></h2>
                    <h5 class="is-size-4 "><a class="has-text-grey" href="<?=base_url().'index.php/posts/'.$post->id?>"><?= $post->subtitle?></a></h5>
                    <div class="is-size-6 " id="profile-post-content" ><a class="has-text-grey" href="<?=base_url().'index.php/posts/'.$post->id?>"><?= $post->post ?></a></div>

                    <div class="prfile-post-footer">
                        <a class="is-link has-text-primary is-size-4  ti-thumb-up"></a> &nbsp; <?= rand(100,1000)?>

                        <div style="margin:0 0 0 auto">
                            <span class="ti-bookmark"></span>

                            <div class="dropdown is-right">
                                <div class="dropdown-trigger">
                                    <a id="edit-post" class="dropdown-btn" aria-haspopup="true" aria-controls="dropdown-menu">
                                        <span class="ti-angle-down has-text-black"></span>
                                    </a>
                                </div>
                                <div class="dropdown-menu" id="dropdown-menu" role="menu">
                                    <div class="dropdown-content">
                                        <a href="<?=base_url().'index.php/posts/'.$post->id.'/edit'?>" class="dropdown-item">
                                            Edit Post 
                                            <span style="" class="ti-pencil is-pulled-right is-size-6"></span>
                                        </a>
                                        <a href="<?=base_url().'index.php/posts/'.$post->id.'/delete'?>" class="dropdown-item">
                                            Delete Post 
                                            <span style="" class="ti-trash is-pulled-right is-size-6"></span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>

                <?php endforeach ?>

            </div>

        </section>

        <footer >
            <div style="margin:auto;">
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Help</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Status</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Writter</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Blog</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Careers</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Privacy</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">Term</a>
                <a class="is-link has-text-grey-light " style="margin: 10px " href="# ">About</a>
            </div>
            <small style="margin:10px auto" class="has-text-grey-light ">© Copyrights 2018 | All Right Reserved | <a class="has-text-grey-light" href="https://arif-un.github.io" target="_blank">Arif-Un</a> &nbsp;&nbsp;&nbsp; <a  class="has-text-grey-light" href="https://www.fb.com/arifoverclocker" target="_blank"><span class="ti-facebook"></span></a> &nbsp; <a class="has-text-grey-light" href="https://www.linkedin.com/in/arif-un/" target="_blank"><span class="ti-linkedin"></span></a> </small>
            <div style="margin: 5px" class="is-flex"><a style="margin: auto" href=""><img class="made-with-bulma" src="<?=base_url()?>assets/images/made-with-bulma.png" alt=""></a></div>
        </footer>
       


        <script src="<?=base_url()?>assets/js/custom.js"></script>
    </body>

</html>